...
public class EditAccount {
    ...
    public void show(AdminPanel parent, Button rfshBtn, VBox accountContainer, Konto accData, List<String> data) {
        accId = accData.getIdKonta();
        selectedPosition = data.get(3);

        // Pola do wpisywania danych
        TextField imieField        = new TextField(data.get(0));
        TextField nazwiskoField    = new TextField(data.get(1));
        TextField peselField       = new TextField(data.get(2));
        TextField loginField       = new TextField(accData.getLogin());
        ...
        VBox prawoJazdyBox = createCheckboxInputCard("Prawo jazdy:", licenseNames.toArray(new String[0]), data);
        ...
        editButton.setOnAction(e -> {
            ...
            //  selectedPositionId + " " + haslo + " " + selectedLicensesIDs);
            editAccount(imie, nazwisko, pesel, login, selectedPositionId, haslo, selectedLicensesIDs);
        ...
        HBox dodajBox = new HBox(editButton);
    ...
    private VBox createCheckboxInputCard(String labelText, String[] options, List<String> data) {
        ...
        List<String> ownedCathegories = new ArrayList<>();

        for (int i = 4; i < data.size(); ++i) {
            ownedCathegories.add(data.get(i));
        }
    ...
    //new add 
    private void editAccount(String imie, String nazwisko, String pesel, String login, String stanowiskoId, String haslo, List<Integer> prawoJazdy) {
        ...
        // try to update pracownik
        RequestController rqPracownik = new RequestController("/pracownik/update/" + accId, 2);
        try{
